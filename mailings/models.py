...
class Mailing(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, verbose_name="Статус", default=CREATED
    )
    ...
    recipients = models.ManyToManyField(
        Recipient, verbose_name="Получатели", related_name="recipients"
    )
...
class MailingAttempt(models.Model):
    ...
    date_attempt = models.DateTimeField(
        verbose_name="Дата и время попытки", help_text="yyyy-mm-dd 00:00:00"
    )
    status = models.CharField(
        max_length=20, choices=STATUS_ATTEMPT_CHOICES, verbose_name="Статус"
    )
